...
function createScatterPlot(data) {
    ...
    svg.selectAll('circle')
        ...
        .on('click', function(event, d) {
            const isSelected = d3.select(this).style('fill') === 'red';
            d3.select(this).style('fill', isSelected ? '#69b3a2' : 'red');
        });
    ...
    const brush = d3.brush()
        ...
        .on("brush end", function(event) {
            ...
            const selectedData = data.filter(d => {
                ...
                const cx = xScale(d.salary_in_usd);
                const cy = yScale(d.experience_level) + yScale.bandwidth() / 2;
