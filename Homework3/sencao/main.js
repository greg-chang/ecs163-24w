...
d3.csv("Student Mental health.csv").then(rawData =>{
    ...
    depressionAgeCountData.splice(6, 1);



    

    console.log(depressionAgeCountData);
    ...
    g1.append("text")
        ...
        .text("Number of People that has mental issue by Age (Line Chart) (zoom)"); // Replace with your actual title
    ...
    rects.enter().append("circle")
         ...
         .style("fill", "none")
         .style("pointer-events", "all"); // This ensures the rectangle can catch mouse events for zooming

         let zoom = d3.zoom()
         .scaleExtent([1, 2]) // Adjust as needed
         .on('zoom', (event) => {
    ...
    g3.append("text")
        .attr("x", 50) // Position in the middle of the pie chart
        .attr("y", -teamHeight/1.5 + teamMargin.top+ 20) // Position above the pie chart
        .attr("text-anchor", "middle") // Center the text
        .style("font-size", "16px")
        .text("People Who Took Treatment by Gender (Bar Chart) (selection)"); // Customize your title here
    ...
    const legend = g3.selectAll('.legend') // Selecting elements of class 'legend'
                    ...
                    .append('g')
                    .attr('class', 'legend')
                    .attr('transform', (d, i) => `translate(300,${i * (legendRectSize + legendSpacing)})`)
    ...
    const tooltip = svg.append("text")
            .attr("x", width / 2) // Position in the center; adjust as needed
            .attr("y", 0) // Position at the top; adjust as needed
            .attr("text-anchor", "middle")
            .style("font-size", "16px")
    ...
    arcs.append("path")
        ...
        .on("mouseover", function(event, d) {
            tooltip
                .text(`${d.data.Gender}: ${d.data.TakeTreatment}`) // Set the text to show the data
        ...
        .on("mouseover", function(event, d) {
            tooltip
                .attr("x", event.clientX) // Adjust based on event
                .attr("y", event.clientY) // Adjust based on event
                .text(`${d.data.Gender}: ${d.data.TakeTreatment}`)
    ...
    g4.append("text")
        ...
        .text("Student Mental Health Analysis (Advanced graph: parallel coordinate graph) (filtering)"); // Your title text
    ...
    .style("right", "400px") // Position as needed
    .style("top", "900px") // Position as needed
    .style("padding", "10px");
    ...
    const yearFilterDropdown = container.append("select")
    .attr("id", "yearFilter")
    .attr("name", "yearFilter");

    
    yearFilterDropdown.append("option").attr("value", "all").text("All Years");

    const uniqueYears = Array.from(new Set(rawData.map(d => d.Year))).sort();
    uniqueYears.forEach(year => {
        yearFilterDropdown.append("option")
            .attr("value", year)
            .text(year);
    });
    ...
    function drawPlot3(filteredData) {
        g4.selectAll(".line").remove(); // This removes everything in g4, 
        g4.selectAll(".line")
            .data(filteredData)
            .enter().append("path")
            .attr("class", "line")
            .attr("d", d => d3.line()
                ...
                (dimensions))
            .attr("fill", "none")
            .attr("stroke", d => yearColorScale(d.Year)) // Using year for color coding
            .attr("stroke-width", 1.5)
            .attr("opacity", 0.8);
        
    }
    
    // Setup event listener for the dropdown to filter data based on year
    yearFilterDropdown.on("change", function() {
        const selectedYear = d3.select(this).property("value");
        const filteredData = selectedYear === "all" ? rawData : rawData.filter(d => d.Year.toString() === selectedYear);
        drawPlot3(filteredData);
    });
