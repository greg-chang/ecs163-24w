...
svg.append("g")
    ...
    function brushed(event) {
        ...
        if (!selection) {
            ...
            infoText.text("Drag to select a range");
        } else {
            var [x0, x1] = selection.map(x.invert);
            
            svg.selectAll(".layer").style("opacity", function(d) {
                var withinRange = d.some(point => {
                    var year = point.data.year;
                    return year >= Math.floor(x0) && year <= Math.ceil(x1);
                });
                return withinRange ? 1 : 0.2;
            });
    
            infoText.text(`Selected range: ${Math.floor(x0)} to ${Math.ceil(x1)}`);
        }
    }
